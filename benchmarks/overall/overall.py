...
def get_method_scores(
    benchmark_dataset: datasets.Dataset,
    methods: List[str],
    ...
    for idx, sample in tqdm(
        enumerate(benchmark_dataset), desc="Running benchmark", total=total_rows
    ):
        ...
        try:
            for method in methods:
                ...
                for score_type in score_types:
                    ...
                    averages_by_type[method][score_type][doc_type].append(
                        scores["score"]
                    )

                    if (
                        "by_block" in scores["specific_scores"]
                    ):  # Not all scorers support this
                        for score, gt_block in zip(
                            scores["specific_scores"]["by_block"], gt_blocks
                        ):
                            averages_by_block_type[method][score_type][
                                gt_block["block_type"]
                            ].append(score)
        except Exception as e:
...
@click.option(
    ...
    default="datalab-to/marker_benchmark",
)
@click.option(
    "--out_dataset", type=str, help="Path to the output dataset", default=None
)
@click.option(
    "--methods",
    type=str,
...
@click.option(
    "--use_llm", is_flag=True, help="Use the LLM model for better marker quality."
)
@click.option(
    "--languages",
    type=str,
    help="Comma separated list of languages to use for LLM",
    default=None,
)
def main(
    dataset: str,
    ...
    for method in methods:
        if method not in METHOD_REGISTRY:
            raise ValueError(
                f"Method {method} not allowed.  Allowed methods are {METHOD_REGISTRY.keys()}"
            )
    ...
    for score_type in score_types:
        if score_type not in SCORE_REGISTRY:
            raise ValueError(
                f"Score type {score_type} not allowed.  Allowed types are {SCORE_REGISTRY.keys()}"
            )
    ...
    if languages:
        benchmark_dataset = benchmark_dataset.filter(
            lambda x: x["language"] in languages
        )
    ...
    if "mathpix" in methods:
        artifacts["mathpix_ds"] = datasets.load_dataset(
            "datalab-to/marker_benchmark_mathpix", split="train"
        )

    if "llamaparse" in methods:
        artifacts["llamaparse_ds"] = datasets.load_dataset(
            "datalab-to/marker_benchmark_llamaparse", split="train"
        )

    if "mistral" in methods:
        artifacts["mistral_ds"] = datasets.load_dataset(
            "datalab-to/marker_benchmark_mistral", split="train"
        )
    ...
    result = get_method_scores(
        benchmark_dataset, methods, score_types, artifacts, max_rows=max_rows
    )
    ...
    print_scores(
        ...
        default_score_type=score_types[0],
    )
    ...
    if out_dataset:
        ...
        dataset = build_dataset(
            benchmark_dataset, result, score_types, max_rows=max_rows
        )
