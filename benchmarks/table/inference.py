...
def inference_tables(
    ...
    for i in tqdm(range(iterations), desc="Converting Tables"):
        try:
            ...
            gt_tables = row[
                "tables"
            ]  # Already sorted by reading order, which is what marker returns
            ...
            if len(marker_json) == 0 or len(gt_tables) == 0:
                print("No tables detected, skipping...")
            ...
            if len(marker_tables) != len(gt_tables):
                print("Number of tables do not match, skipping...")
                total_unaligned += len(gt_tables)
                continue
            ...
            marker_areas = [
                (bbox[2] - bbox[0]) * (bbox[3] - bbox[1]) for bbox in marker_table_boxes
            ]
