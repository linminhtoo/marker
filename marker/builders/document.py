...
class DocumentBuilder(BaseBuilder):
    ...
    def build_document(self, provider: PdfProvider):
        ...
        highres_images = provider.get_images(
            provider.page_range, self.highres_image_dpi
        )
        initial_pages = [
            PageGroupClass(
                page_id=p,
        ...
        source_path = getattr(provider, "source_filepath", provider.filepath)
        return DocumentClass(filepath=source_path, pages=initial_pages)
